...
void initMotors()
{
	//specify the speed for (objects,screen)
		...
		reset_phidgets_linear_ZaxisOnly(init_phidget_distance);
...
*/
			...
			case trial_handReturn:
				...
				text.draw("# linAct_measured_distance: " + stringify<double>(linAct_measured_distance));
				text.draw("# phidget_position_read: " + stringify<double>(phidget_position_read/10.0));
				text.draw("# net_phidget_distance: " + stringify<double>(net_phidget_distance));
...
void change_haptic_object(double hapticDepth){
	...
	if(abs(net_phidget_distance - prev_phidget_ditance)> 2){
		...
		phidget_position_read = phidgets_linear_move_precise(net_phidget_distance, kit_plugIn_ID, min_phidge_distance);
		prev_phidget_ditance = net_phidget_distance;
	}else{
		phidget_position_read = phidgets_linear_move_precise(net_phidget_distance + 6, kit_plugIn_ID, min_phidge_distance);
		phidget_position_read = phidgets_linear_move_precise(net_phidget_distance, kit_plugIn_ID, min_phidge_distance);
		prev_phidget_ditance = net_phidget_distance;
	}
...
int check_linAct(double offsetAllowed){
	int linAct_measured_position;
	linAct_measured_position = phidgets_linear_getCurrentPos(kit_plugIn_ID);
	linAct_measured_distance = linAct_measured_position/10.0;
	
	double linAct_est_distance = (linAct_measured_distance + phidget_position_read/10.0)/2.0;

	if(abs(linAct_est_distance - net_phidget_distance) > offsetAllowed){
		return 1;
	}else{
		return 0;
	}
...
void check_haptic(){
	...
	if(current_error_state == blocked_objMarkers){
		...
		*/
		...
		if(abs(centeredObj_y_aiming - centeredObj_y_now) > 2){
			beepOk(24);
	...
	*/
	...
	case trial_fixate:
		if (ElapsedTime > fixateTime) {	
			...
			if(linActRedFlag > 0){
					phidget_position_read = phidgets_linear_move_precise(net_phidget_distance + 5, kit_plugIn_ID, min_phidge_distance);
					phidget_position_read = phidgets_linear_move_precise(net_phidget_distance, kit_plugIn_ID, min_phidge_distance);
					prev_phidget_ditance = net_phidget_distance;
					liinAct_correction++;
			}
	...
	case trial_visualStimulus:
		...
		if(testHapticDevice){			
			...
			// if going to object for grasps
			if(dist_thm_target < thresholdDist_near_target){
				beepOk(23);
			}
			// ready for MSE?
			//if(handNearHome && gripSmall){
			if((dist_thm_home < thresholdDist_near_home) && (grip_aperture < thresholdGA_small)){
				beepOk(21);
		...
		case '+':
			switch(current_stage){
				...
				if(!testHapticDevice){
					...
					if((grip_aperture < 200) && (dist_thm_home < thresholdDist_near_home) && (abs(vel_grip_change) < thresholdVelGA_steady)){
...
void beepOk(int tone)
{
	...
	case 5: //"go"
		PlaySound((LPCSTR) "C:\\cygwin\\home\\visionlab\\workspace\\cncsvision\\data\\beep\\spoken-go.wav", NULL, SND_FILENAME | SND_ASYNC);
	break;
	...
	case 20: // alighnment
	PlaySound((LPCSTR) "C:\\cygwin\\home\\visionlab\\workspace\\cncsvision\\data\\beep\\spoken-alignment.wav",
		NULL, SND_FILENAME | SND_ASYNC);
	break;

	case 21: // estimate
	PlaySound((LPCSTR) "C:\\cygwin\\home\\visionlab\\workspace\\cncsvision\\data\\beep\\spoken-estimate-short.wav",
		NULL, SND_FILENAME | SND_ASYNC);
	break;

	case 22: // grasp
	PlaySound((LPCSTR) "C:\\cygwin\\home\\visionlab\\workspace\\cncsvision\\data\\beep\\spoken-grasp-short.wav",
		NULL, SND_FILENAME | SND_ASYNC);
	break;
	...
	case 24: // help
	PlaySound((LPCSTR) "C:\\cygwin\\home\\visionlab\\workspace\\cncsvision\\data\\beep\\spoken-help.wav",
		NULL, SND_FILENAME | SND_ASYNC);
	break;
	}
	return;
